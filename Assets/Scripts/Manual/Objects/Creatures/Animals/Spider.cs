...
public class Spider : MonoBehaviour
{
    ...
    private void Update()
    {
        transform.localEulerAngles = new Vector3(transform.localEulerAngles.x, transform.localEulerAngles.y, (LookingDr - transform.localEulerAngles.z) * 0.1f + transform.localEulerAngles.z);
        if (WalkTrue)
        {
            Vector3 PossiblePos = Vector3.Lerp(transform.localPosition, transform.up + transform.localPosition, Speed);
